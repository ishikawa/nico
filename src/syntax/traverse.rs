...
pub trait Visitor {
    ...
    // Node
    fn enter_program(&mut self, path: &mut NodePath, program: &mut Program) {}
    fn exit_program(&mut self, path: &mut NodePath, program: &mut Program) {}
    ...
    fn exit_identifier(&mut self, path: &mut NodePath, id: &mut Identifier) {}

    fn enter_struct_definition(&mut self, path: &mut NodePath, definition: &mut StructDefinition) {}
    fn exit_struct_definition(&mut self, path: &mut NodePath, definition: &mut StructDefinition) {}
    ...
    fn exit_function_parameter(&mut self, path: &mut NodePath, param: &mut FunctionParameter) {}

    fn enter_type_field(&mut self, path: &mut NodePath, field: &mut TypeField) {}
    fn exit_type_field(&mut self, path: &mut NodePath, field: &mut TypeField) {}

    fn enter_type_annotation(&mut self, path: &mut NodePath, annotation: &mut TypeAnnotation) {}
    fn exit_type_annotation(&mut self, path: &mut NodePath, annotation: &mut TypeAnnotation) {}

    fn enter_statement(&mut self, path: &mut NodePath, statement: &mut Statement) {}
    fn exit_statement(&mut self, path: &mut NodePath, statement: &mut Statement) {}
    ...
    fn enter_pattern(&mut self, path: &mut NodePath, pattern: &mut Pattern) {}
    fn exit_pattern(&mut self, path: &mut NodePath, pattern: &mut Pattern) {}

    fn enter_value_field(&mut self, path: &mut NodePath, field: &mut ValueField) {}
    fn exit_value_field(&mut self, path: &mut NodePath, field: &mut ValueField) {}

    fn enter_value_field_pattern(&mut self, path: &mut NodePath, pattern: &mut ValueFieldPattern) {}
    fn exit_value_field_pattern(&mut self, path: &mut NodePath, pattern: &mut ValueFieldPattern) {}

    fn enter_expression(&mut self, path: &mut NodePath, expression: &mut Expression) {}
    fn exit_expression(&mut self, path: &mut NodePath, expression: &mut Expression) {}

    fn enter_integer_literal(&mut self, path: &mut NodePath, expr: &mut Expression, literal: i32) {}
    fn exit_integer_literal(&mut self, path: &mut NodePath, expr: &mut Expression, literal: i32) {}
    ...
    fn exit_variable(&mut self, path: &mut NodePath, expr: &mut Expression, id: &mut Identifier) {}
...
pub fn traverse(visitor: &mut dyn Visitor, program: &mut Program) {
    let path = wrap(NodePath::new(NodeKind::Program(program)));
    traverse_path(visitor, &path);
}

fn traverse_path(visitor: &mut dyn Visitor, path: &'a Rc<RefCell<NodePath>>) {
...
fn dispatch_enter(visitor: &mut dyn Visitor, path: &'a Rc<RefCell<NodePath>>) {
            ...
            if !path.skipped() {
                match expr.kind() {
                    ...
                    ExpressionKind::VariableExpression(kind) => {
                        visitor.enter_variable(&mut path, *expr, *kind);
                    }
...
fn dispatch_exit(visitor: &mut dyn Visitor, path: &'a Rc<RefCell<NodePath>>) {
    let mut path = &mut path.borrow_mut();
...
fn traverse_children(visitor: &mut dyn Visitor, path: &'a Rc<RefCell<NodePath>>) {
...
mod tests {
    ...
    impl Visitor for NodeCounter {
        fn enter_expression(&mut self, _path: &mut NodePath, _expr: &mut Expression) {
